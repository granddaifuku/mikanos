...
Error HIDBaseDriver::OnEndpointsConfigured() {
  ...
  initialize_phase_ = 1;
  return ParentDevice()->ControlOut(kDefaultControlPipeID, setup_data, nullptr,
                                    0, this);
}
...
Error HIDBaseDriver::OnControlCompleted(EndpointID ep_id, SetupData setup_data,
  ...
  Log(kDebug,
      "HIDBaseDriver::OnControlCompleted: dev %08x, phase = %d, len = %d\n",
...
Error HIDBaseDriver::OnInterruptCompleted(EndpointID ep_id, const void* buf,
                                          int len) {
  if (ep_id.IsIn()) {
    OnDataReceived();
    std::copy_n(buf_.begin(), len, previouse_buf_.begin());
    return ParentDevice()->InterruptIn(ep_interrupt_in_, buf_.data(),
                                       in_packet_size_);
  }
