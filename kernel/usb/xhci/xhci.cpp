...
// root hub port
//はリセット処理をしてからアドレスを割り当てるまでは他の処理を挟まず。そのポートについての処理だけをしなければならない。kWaitingAddressed
// はリセット（kResettingPort）からアドレス割り当て（kAddressingDevice）までの一連の処理の実行を待っている状態。

std::array<volatile ConfigPhase, 256> port_config_phase{}; // index: port number

// kResettingPort から kAddressingDevice までの処理を実行中のポート番号。 0
// ならその状態のポートがないことを示す。
